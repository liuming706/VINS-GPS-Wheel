...
bool LoadSensorData(string& sensor_data_file,
                    unordered_map<string, string>* time_data_map) {
...
int main(int argc, char* argv[]) {
  if (argc != 2) {
    cerr
        << "./kaist_pub "
           "PATH_TO_FOLDER/YOUR_PATH_TO_DATASET/KAIST/urban28/urban28-pankyo \n"
        << "For example: ./kaist_pub "
           "/home/wallong/dataset/urban28/urban28-pankyo"
        << endl;
    return -1;
  }
  sData_path = argv[1];
  ...
  img_publisher =
      nh.advertise<sensor_msgs::Image>("/stereo/left/image_raw", 100, true);
  encoder_publisher =
      nh.advertise<custom_msgs::Encoder>("/encoder/data_raw", 100, true);
  gps_publisher =
      nh.advertise<sensor_msgs::NavSatFix>("/gps/data_raw", 100, true);
  ...
  while (getline(file_data_stamp, line_str) && ros::ok()) {
    ...
    if (sensor_type == "stereo") {
      const string img_file =
          sData_path + "/image/stereo_left/" + time_str + ".png";
      ...
      sensor_msgs::ImagePtr img_msg =
          cv_bridge::CvImage(std_msgs::Header(), "mono8", gray_img)
              .toImageMsg();
      ;
    ...
    if (sensor_type == "encoder") {
      if (time_encoder_map.find(time_str) == time_encoder_map.end()) {
        ROS_ERROR("[PublishData]: Failed to find encoder data at time: %s",
                  time_str);
      }
    ...
    if (sensor_type == "gps") {
      if (time_gps_map.find(time_str) == time_gps_map.end()) {
        ROS_ERROR("[PublishData]: Failed to find gps data at time: %s",
                  time_str);
      }
